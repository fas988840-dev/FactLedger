...
A read-only Solana wallet intelligence API that never fabricates data. Every value is either verified on-chain or returned as null — never estimated. Every score cites the exact transactions behind it, so it can be independently recomputed. Open-source (MIT), deployed live at factledger-api.onrender.com.
